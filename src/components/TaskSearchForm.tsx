...
export default function TaskSearchForm() {
  ...
  const genJSONSheet = async (usrId) => {
    let arrMst = [];
    let _newLst = [];
    //Sheet Start
      // Initialize the sheet - doc ID is the long id in the sheets URL
    const doc = new GoogleSpreadsheet(CPS_SHEET_ID.spreadID); //script data
      ...
      if(mst_sub_cd) {
        if(no && parseInt(no) > 0 && isUse && isUse == 'Yes') {
          ...
          if(mst_cd != null && mst_nm != "" && mst_sub_cd != null && mst_sub_nm != "") {
            ...
            _item.code = _flag.code;
            _item.name = _flag.name;
          }
    ...
    console.log("-------arrMst", _newLst);
   // Remove duplicates from the array based on 'code' property (or any other unique key)
    const uniqueArray = removeDuplicates(_newLst, 'code');
    ...
    console.log("-------uniqueArray", uniqueArray);
      ...
      // Group objects by 'code' property
    const groupedArray = groupByCode(_newLst);
  ...
  // Function to remove a specific property from an object
              ...
              <tr className="bg-gray-200">
                ...
                <th className="px-2 py-2 text-right">
                  <div className="grid grid-flow-col text-center">
                    ...
                    <button type="button" className="bg-green text-white py-2 px-2 rounded-lg" 
                      onClick={event => genJSONSheet('namnnguyen')}>
                      Master Code
                    </button>
                  </div>
